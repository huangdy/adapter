...
import com.spotonresponse.adapter.mode.MappedRecord;

public class XCoreWebServiceClient {

    private static final Logger logger = LoggerFactory.getLogger(XCoreWebServiceClient.class);
    ...
    public static void setWebServiceTemplate(WebServiceTemplate webServiceTemplate) {
        ...
        XCoreWebServiceClient.webServiceTemplate = webServiceTemplate;
    }

    public XCoreWebServiceClient() {

        super();
    }

    public XCoreWebServiceClient(String uri, String username, String password) {
        ...
        logger.debug("XCoreWebServiceClient: url: " + uri + ", username: " + username);
    ...
    public boolean createIncident(MappedRecord record) {
        ...
        request.addNewCreateIncidentRequest().addNewIncident().set(XCoreUtil.getIncidentDocument(
            record));
        ...
        if (response != null &&
            ...
            record.setIgID(XCoreUtil.getIGID(workProduct));
            record.setWorkProductID(XCoreUtil.getProductIdentification(workProduct));
    ...
    private XmlObject sendAndReceive(XmlObject request) {
        ...
        try {
            ...
            logger.error("XCoreWebServiceClient.marshalSendAndReceive: " + e.getMessage());
        }
        return response;
    }
    ...
    public boolean updateIncident(MappedRecord record) {
        ...
        request.addNewUpdateIncidentRequest().addNewIncident().set(XCoreUtil.getIncidentDocument(
            record));
        request.getUpdateIncidentRequest().addNewWorkProductIdentification().set(
            XCoreUtil.getProductIdentification(record.getWorkProductID()));
